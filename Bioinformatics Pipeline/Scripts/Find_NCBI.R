FindNCBI <- function(species_list,locus,setmax = 100, group = T, acc_only = F, summ_only = F){
  require(rentrez)
  ...
  for(i in 1:nrow(species_list)){
    ...
    my.term <- paste0("(",species_list$sci.name[i],"[Organism]) AND ",locus,"[Title]")
    my.term
    ...
    if(acc_only == F){
      if(length(x$ids) > 0){
        ...
        write.table(my.fetch, file = paste0("Output/seqs/",species_list$sci.name[i],"_",locus,".fasta"), quote = F, row.names = F, col.names = F)
      }
    }
    if(acc_only == T){
      if(length(x$ids > 0)){
  ...
  if(group == T){
    my.file <- list.files("Output/seqs/", pattern = locus)
  ...
  write.table(x = species_list,file = "Output/trnL_community.summary.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
}