...
df<-read.csv("reads.spp.groups.csv",header=TRUE)
...
p2<-ggplot(df, aes(fill=habitat, y=species.richness, x=lake.name)) + 
  ...
  theme_classic()+theme(axis.text.x = element_text(angle = 60,hjust = 1, size=8),
                        ...
                        axis.title.y = element_text(size=18,
                                                    margin = margin(t=0,r=20,b=0,l=0)))+
  scale_fill_brewer(palette="Set2")+ylab("Species richness")


combinedPlot <- plot_grid(p2, p1, ncol=2, align='hv', rel_widths=1,
                          rel_heights= 1)

png("Proportions per lake.png",width = 3500, height = 1000, units = "px", res=300)
combinedPlot
dev.off()
