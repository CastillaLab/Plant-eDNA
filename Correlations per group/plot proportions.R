...
p2<-ggplot(df,aes(x=terrestrial.spp, y=aquatic.spp))+
  geom_smooth(method = "lm", se=FALSE, col = "black", size=0.8)+
  geom_point(shape=21,size=3)+theme_bw()+
  xlab("Richness Terrestrial Species")+
  ylab("Richness Aquatic Species")+
  theme(axis.title = element_text(size=16),
        axis.text = element_text(size=14))


p1<-ggplot(df,aes(x=wetland.spp, y=aquatic.spp))+
  geom_smooth(method = "lm", se=FALSE, col = "black", size=0.8)+
  geom_point(shape=21,size=3)+theme_bw()+
...
combinedPlot <- plot_grid(p1, p2, p3, p4,p5, p6, ncol=2, nrow=3, align='hv',
